...
void Camera::setViewDirection(glm::vec3 position, glm::vec3 direction, glm::vec3 up) {
    ...
    const glm::vec3 w{glm::normalize(direction)};   //  camera's view direction unit
    const glm::vec3 u{glm::normalize(glm::cross(w, up))};   //  camera's right direction unit
    const glm::vec3 v{glm::cross(w, u)};    //  camera's up direction unit
...
void Camera::setViewTarget(glm::vec3 position, glm::vec3 target, glm::vec3 up) {
    //assert((target-position).length() == 0);
    setViewDirection(position, target - position, up);
}

//  camera movement & rotation -> Trait-Bryan Euler Angle YXZ
